  ...
  <body>
    ...
    <div class="col-lg-12 text-center v-center">
      ...
      <form class="col-lg-12" action="index.php" method="post">
        ...
        <?php $selectedSearch = $_POST['find'];
        switch ($selectedSearch) {
          case 'title':
              $sql = "title LIKE '%$query%'";
              break;
          case 'director':
              $sql = "director LIKE '%$query%'";
              break;
          case 'actor':
              $sql="actor LIKE '%$query%'";
              break;
          case 'all':
              $sql="title LIKE '%$query%' OR actor LIKE '%$query%' OR director LIKE '%$query%'";
              break;
        } ?>
        <input type="text" name="search" placeholder="키워드를 입력하세요." />
        <input type="submit" name="submit" value="검색" />
        <!-- <input name="title" id="search" class="form-control input-lg" placeholder="키워드 입력" type="text">
          <span class="input-group-btn"><button id="button" class="btn btn-lg btn-primary" type="submit">OK</button></span> -->
        </div>
        <?php
        if(isset($_POST['search'])){
          $searchq = $_POST['search'];
        //  $query = mysqli_query($conn,"SELECT * FROM MovieInfoList WHERE $sql ")
        } ?>
      </form>
      </div>
    </div> <!-- /row -->
    ...
    <?php
    $result = mysqli_query($conn,"SELECT * FROM MovieInfoList ORDER BY $selectedOption $order LIMIT 6");
    if($result === false){
      $selected = 'is_rated';
      echo '정렬기준을 선택하세요';
    }
